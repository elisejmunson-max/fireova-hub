...
function AssetPanel({ asset, folders, onClose, onArchive, onRestore, onDeletePermanently, onUpdate, getDisplayUrl, isImage, isVideo, canPreview, isArchiveView }: PanelProps) {
  ...
  const [analysisTagInput, setAnalysisTagInput] = useState('')
  ...
  const [mediaAnalysis, setMediaAnalysis] = useState<MediaAnalysis | null>(() => readMediaAnalysis(asset.id))
  ...
  if (assetId !== lastId) {
    ...
    setMediaAnalysis(readMediaAnalysis(asset.id))
  ...
  async function analyzeAsset(force = false) {
    if (analysisRunning) return

    setAnalysisRunning(true)
    try {
      const report = await analyzeMediaItem({
        media: createMockMediaFromAsset(asset, getDisplayUrl),
        force,
      })
      setMediaAnalysis(report)
    } finally {
      setAnalysisRunning(false)
    }
  }
  ...
  function approveSuggestions() {
    const report = approveAllMediaAnalysisSuggestions(asset.id)
    if (report) setMediaAnalysis(report)
  }

  function rejectSuggestion(suggestionId: string) {
    const report = rejectMediaAnalysisSuggestion(asset.id, suggestionId)
    if (report) setMediaAnalysis(report)
  }

  function addAnalysisTag() {
    const report = addUserMediaAnalysisTag(asset.id, analysisTagInput)
    setAnalysisTagInput('')
    if (report) setMediaAnalysis(report)
  }
    ...
    <div className="fixed inset-y-0 right-0 w-80 bg-white border-l border-stone-200 z-20 overflow-y-auto shadow-warm-md">
      <div className="p-5">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="rounded-lg bg-stone-50 p-3 ring-1 ring-stone-100">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-ember-600">Suggested by your Marketing Director</p>
                <p className="mt-1 text-xs leading-5 text-stone-500">
                  {mediaAnalysis ? `Status: ${mediaAnalysis.status}` : 'No analysis has been generated for this asset.'}
                </p>
              </div>
              <button
                type="button"
                onClick={() => analyzeAsset(Boolean(mediaAnalysis))}
                disabled={analysisRunning}
                className="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-stone-700 ring-1 ring-stone-200 transition-colors hover:bg-stone-100 disabled:cursor-wait disabled:opacity-60"
              >
                {analysisRunning ? 'Analyzing...' : mediaAnalysis ? 'Analyze Again' : 'Analyze'}
              </button>
            </div>
            ...
            {mediaAnalysis && (
              <div className="mt-3 space-y-3">
                {mediaAnalysis.error && (
                  <div className="rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 ring-1 ring-red-100">
                    {mediaAnalysis.error.message}
                  </div>
                )}

                {mediaAnalysis.visualObservations.length > 0 && (
                  <div>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Visual Summary</p>
                    <ul className="space-y-1 text-xs leading-5 text-stone-700">
                      {mediaAnalysis.visualObservations.slice(0, 3).map((observation) => (
                ...
                {mediaAnalysis.suggestedCategory && (
                  <div>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Suggested Category</p>
                    <span className="inline-flex rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-stone-700 ring-1 ring-stone-200">
                      {mediaAnalysis.suggestedCategory}
                    </span>
                  </div>
                )}
                ...
                <SuggestionPills
                  ...
                  suggestions={mediaAnalysis.suggestedContentThemes}
                  rejectedIds={mediaAnalysis.userReview.rejectedSuggestionIds}
                  onReject={rejectSuggestion}
                />

                <SuggestionPills
                  title="Suggested Tags"
                  suggestions={mediaAnalysis.suggestedTags}
                  rejectedIds={mediaAnalysis.userReview.rejectedSuggestionIds}
                  onReject={rejectSuggestion}
                />

                {mediaAnalysis.marketingUses.length > 0 && (
                  <div>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Recommended Marketing Uses</p>
                    <div className="flex flex-wrap gap-1.5">
                      {mediaAnalysis.marketingUses.slice(0, 4).map((use) => (
                        <span key={use.id} className="rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-stone-700 ring-1 ring-stone-200">
                          {use.use}
                        </span>
                      ))}
                    </div>
                ...
                {mediaAnalysis.quality.notes.length > 0 && (
                  <div>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Quality Notes</p>
                    <p className="text-xs leading-5 text-stone-600">{mediaAnalysis.quality.notes[0]}</p>
                  </div>
                )}

                {mediaAnalysis.representativeFrames && mediaAnalysis.representativeFrames.length > 0 && (
                  <div>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Representative Frames</p>
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {mediaAnalysis.representativeFrames.map((frame) => (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          key={frame.id}
                          src={frame.localAssetReference}
                          alt={`Frame at ${frame.timestampSeconds}s`}
                          className="h-12 w-16 rounded-md object-cover ring-1 ring-stone-200"
                        />
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-lg bg-stone-950 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-stone-800"
                  >
                    Approve Suggestions
                  </button>
                  <button
                    ...
                    className="rounded-lg bg-white px-3 py-2 text-xs font-semibold text-stone-700 ring-1 ring-stone-200 transition-colors hover:bg-stone-100"
                  >
                    Edit
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={analysisTagInput}
                    onChange={(event) => setAnalysisTagInput(event.target.value)}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                    ...
                    placeholder="Add reviewed tag..."
                    className="input flex-1 py-1.5 text-xs"
                  />
                  <button
                    type="button"
                    onClick={addAnalysisTag}
                    disabled={!analysisTagInput.trim()}
                    className="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-stone-700 ring-1 ring-stone-200 transition-colors hover:bg-stone-100 disabled:opacity-40"
                  >
                    Add
                  </button>
                </div>

                {mediaAnalysis.userReview.userAddedTags.length > 0 && (
                  <p className="text-xs leading-5 text-stone-500">
                    User added: {mediaAnalysis.userReview.userAddedTags.join(', ')}
                  </p>
                )}
                ...
                {process.env.NODE_ENV !== 'production' && (
                  <div>
                    <button
                      ...
                      className="text-xs font-semibold text-stone-500 underline underline-offset-4"
                    >
                      {showInspector ? 'Hide intelligence inspector' : 'View analysis inspector'}
                    </button>
                    {showInspector && (
                      <pre className="mt-2 max-h-60 overflow-auto rounded-lg bg-stone-950 p-3 text-[10px] leading-4 text-stone-100">
                        {JSON.stringify({
                          analysisVersion: mediaAnalysis.analysisVersion,
                          providerSource: mediaAnalysis.source,
                          sourceFingerprint: mediaAnalysis.sourceFingerprint,
                          suggestions: {
                            observations: mediaAnalysis.visualObservations,
                            subjects: mediaAnalysis.detectedSubjects,
                            actions: mediaAnalysis.detectedActions,
                            foodItems: mediaAnalysis.detectedFoodItems,
                            businessElements: mediaAnalysis.detectedBusinessElements,
                            eventSignals: mediaAnalysis.detectedEventSignals,
                            tags: mediaAnalysis.suggestedTags,
                            contentThemes: mediaAnalysis.suggestedContentThemes,
                          },
                          reviewState: mediaAnalysis.userReview,
                          errorState: mediaAnalysis.error,
                        }, null, 2)}
                      </pre>
                    )}
                  </div>
...
function SuggestionPills({
  ...
  title: string
  suggestions: Array<{ id: string; label: string; confidence: string }>
  rejectedIds: string[]
  onReject: (suggestionId: string) => void
}) {
  const visibleSuggestions = suggestions.filter((suggestion) => !rejectedIds.includes(suggestion.id))
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {visibleSuggestions.map((suggestion) => (
          <span key={suggestion.id} className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-stone-700 ring-1 ring-stone-200">
            {suggestion.label}
            <button
              type="button"
...
function createMockMediaFromAsset(asset: AnyAsset, getDisplayUrl: (a: AnyAsset) => string): MockMedia {
  return {
    id: asset.id,
    type: asset.file_type.startsWith('video/') ? 'video' : 'photo',
    src: getDisplayUrl(asset),
    alt: [asset.filename, ...(asset.tags ?? []), asset.notes ?? ''].filter(Boolean).join(' '),
  }
}
