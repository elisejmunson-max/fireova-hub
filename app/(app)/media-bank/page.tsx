  ...
  if (user && user.id !== 'dev') {
    if (eventId) {
      const { data: eventMedia } = await supabase
        .from('event_media').select('id,storage_path,file_name,file_type,size_bytes,created_at')
        .eq('user_id', user.id).eq('event_id', eventId).order('created_at', { ascending: true })
      if (eventMedia?.length) await supabase.from('media_assets').upsert(eventMedia.map((item: any) => ({
        id: item.id, user_id: user.id, filename: item.file_name, storage_path: item.storage_path,
        file_type: item.file_type, size_bytes: item.size_bytes ?? 0, tags: [`event:${eventId}`], notes: null, created_at: item.created_at,
      })), { onConflict: 'id', ignoreDuplicates: true })
      const { data, error } = await supabase.from('media_assets').select('*').contains('tags', [`event:${eventId}`]).order('created_at', { ascending: true }).limit(500)
