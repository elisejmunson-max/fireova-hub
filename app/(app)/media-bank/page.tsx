...
import PhotoPolishTest from './photo-polish-test'
...
export default async function MediaBankPage({ searchParams }: { searchParams?: { eventId?: string } }) {
    ...
    if (eventId) {
      ...
      if (eventMedia?.length) {
        await supabase.from('media_assets').upsert(eventMedia.map((item: any) => ({
          id: item.id, user_id: user.id, filename: item.file_name,
          storage_path: item.storage_path, file_type: item.file_type,
          size_bytes: item.size_bytes ?? 0, tags: [`event:${eventId}`],
          notes: null, created_at: item.created_at,
        })), { onConflict: 'id', ignoreDuplicates: true })
      }

      const { data, error } = await supabase
        .from('media_assets').select('*').contains('tags', [`event:${eventId}`])
  ...
  return <div>
    {eventId && assets.length === 0 && <div className="card mb-6 p-6 text-center">
    ...
    {eventId && assets.length > 0 && <PhotoPolishTest assets={assets as any[]} />}
