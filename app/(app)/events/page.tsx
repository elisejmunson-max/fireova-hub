...
const COCKTAIL_SECTIONS: {
...
}[] = [
  {
    ...
    unit: '',
    items: [
      { name: 'Grazing Table', sizes: ['3ft', '4ft', '6ft', '8ft'], colors: ['White', 'Wood'] },
                ...
                {newName && (
                  <div className="space-y-2">
                    ...
                    {selectedDef?.colors && (
                      ...
                      >
                        <option value="">— Setup color —</option>
...
const PACK_LISTS: Record<string, string[]> = {
  ...
  'Roasted Tomato Crostini': [
    ...
    'Arugula',
    'Marinated tomatoes',
    'Sheet pan for tomatoes / crostini',
    'Balsamic glaze',
    'Rectangle platter',
  ],
  'Stuffed Mushrooms': [
      ...
      <div className="divide-y divide-stone-100">
        {hasCocktail && (
          <div className="px-4 py-3 space-y-1.5">
            ...
            {(() => {
              const groups: { section: string; items: typeof cocktailItems }[] = []
              ...
              return groups.map((group) => (
                <div key={group.section}>
                  ...
                  {group.items.map((item) => {
                    const idx = cocktailItems.indexOf(item)
                    const label = formatCocktailLabel(item.name, item.qty)
                    return <CheckRow key={`cocktail-${idx}`} label={label} checkKey={`cocktail-${idx}`} />
                  })}
                </div>
              ))
            })()}
