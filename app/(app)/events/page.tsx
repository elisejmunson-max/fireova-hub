...
function MenuNotesTab({
  ...
  return (
    <div className="p-6 max-w-3xl space-y-6">
      ...
      <section>
        <h3 className="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-3">Menu Selection</h3>
        <p className="text-xs text-stone-400 mb-4">Pick items for this event. Pack list updates automatically.</p>
        <div className="space-y-3">
          {MENU_CATEGORIES.map((cat) => {
            const selected = cat.items.filter((i) => selectedMenuItems.includes(i))
            return (
              <div key={cat.label} className="bg-white border border-stone-200 rounded-xl overflow-hidden">
                <details className="group">
                  <summary className="flex items-center justify-between px-4 py-3 cursor-pointer list-none select-none hover:bg-stone-50 transition-colors">
                    <div className="flex items-center gap-2">
                    ...
                    <svg className="w-4 h-4 text-stone-400 group-open:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div className="divide-y divide-stone-100 border-t border-stone-100">
                    {cat.items.map((item) => {
                      ...
                      return (
                        <label key={item} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-stone-50 transition-colors">
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={() => onToggleMenu(item)}
                            className="w-4 h-4 rounded border-stone-300 text-ember-600 focus:ring-ember-500"
                          />
                          <span className={`text-sm ${checked ? 'text-stone-900 font-medium' : 'text-stone-600'}`}>{item}</span>
                        </label>
                      )
                    })}
                  </div>
                ...
                {selected.length > 0 && (
                  <div className="px-4 py-2 bg-stone-50 border-t border-stone-100 flex flex-wrap gap-1.5">
                    {selected.map((item) => (
                      <span key={item} className="flex items-center gap-1 pl-2 pr-1 py-0.5 text-xs font-medium text-ember-700 bg-ember-50 border border-ember-200 rounded-full">
                        {item}
                        <button type="button" onClick={() => onToggleMenu(item)} className="w-3.5 h-3.5 rounded-full hover:bg-ember-200 flex items-center justify-center transition-colors">
