...
function editedFilename(name: string) { const base = name.replace(/\.[^.]+$/, ''); return `${base}-polished.jpg` }
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  let output: Buffer
  try { output = await sharp(Buffer.from(await original.arrayBuffer())).rotate().modulate({ brightness: 1.035, saturation: 1.025 }).linear(1.035, -2).sharpen({ sigma: 0.75, m1: 0.8, m2: 1.2 }).jpeg({ quality: 92, chromaSubsampling: '4:4:4', mozjpeg: true }).toBuffer() }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Photo processing failed.' }, { status: 500 }) }
  const newId = crypto.randomUUID(), folder = asset.storage_path.includes('/') ? asset.storage_path.slice(0, asset.storage_path.lastIndexOf('/')) : `${user.id}/polished`, newName = editedFilename(asset.filename || 'photo.jpg'), newPath = `${folder}/${newId}-${newName}`
  ...
  const row = { id:newId,user_id:user.id,filename:newName,storage_path:newPath,file_type:'image/jpeg',size_bytes:output.length,tags,notes:'Professional polish created from original. Safe tonal and sharpness adjustments only.',created_at:new Date().toISOString(),folder_id:asset.folder_id??null,ai_status:null,ai_reason:null,ai_categories:asset.ai_categories??[],ai_post_uses:asset.ai_post_uses??[],ai_edit_suggestion:null }
