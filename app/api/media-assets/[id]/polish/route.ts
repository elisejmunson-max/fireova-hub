...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    output = await sharp(input)
      ...
      .linear(1.035, -2)
      .sharpen({ sigma: 0.75, m1: 0.8, m2: 1.2 })
  ...
  const { error: uploadError } = await supabase.storage.from('media').upload(newPath, output, {
  ...
  const row = {
    ...
    ai_status: 'pending',
    ai_reason: 'Edited copy is ready for AI re-review.',
    ai_categories: asset.ai_categories ?? [],
    ai_post_uses: asset.ai_post_uses ?? [],
    ai_edit_suggestion: null,
  }
  ...
  const { data: eventSource } = await supabase
    .from('event_media')
    .select('event_id,media_kind')
    .eq('id', asset.id)
    .eq('user_id', user.id)
    .maybeSingle()
  if (eventSource?.event_id) {
    await supabase.from('event_media').insert({
      ...
      size_bytes: output.length,
      thumbnail_path: null,
      preview_url: supabase.storage.from('media').getPublicUrl(newPath).data.publicUrl,
      checksum: null,
      metadata: { editedFrom: asset.id, editType: 'professional-polish' },
    })
  }

  const publicUrl = supabase.storage.from('media').getPublicUrl(newPath).data.publicUrl
  return NextResponse.json({
    original: { id: asset.id, filename: asset.filename, storage_path: asset.storage_path },
    edited: { ...row, public_url: publicUrl },
  })
}
