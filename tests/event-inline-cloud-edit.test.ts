...
test('the complete labeled Event Details form is the default and only layout', () => {
  ...
  assert.match(inlineHeaderSource, /placeholder="Name of event"/)
  assert.match(inlineHeaderSource, /md:grid-cols-3/)
  assert.match(inlineHeaderSource, /text-\[26px\] font-semibold[\s\S]*?text-\[#171717\][\s\S]*?md:text-\[32px\]/)
  assert.match(inlineHeaderSource, /placeholder:text-\[24px\] placeholder:font-normal placeholder:text-\[#A8A29E\]/)
  assert.match(inlineHeaderSource, /md:placeholder:text-\[28px\]/)
...
test('name autosaves once after 700ms of inactivity and flushes immediately on blur', () => {
  ...
  assert.doesNotMatch(inlineHeaderSource, /updates\.name = ['"]Name of event['"]/)
})
