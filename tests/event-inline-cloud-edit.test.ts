...
test('the complete labeled Event Details form is the default and only layout', () => {
  ...
  assert.match(inlineHeaderSource, /absolute left-4 top-1\/2[\s\S]*?<CalendarIcon \/>/)
  assert.match(inlineHeaderSource, /cursor-pointer[\s\S]*?pl-12 pr-10 text-left/)
  assert.match(inlineHeaderSource, /absolute right-4 top-1\/2[\s\S]*?>⌄<\/span>/)
  assert.match(inlineHeaderSource, /grid-cols-\[minmax\(0,1fr\)_minmax\(0,1fr\)\] gap-3 md:grid-cols-3/)
  assert.match(inlineHeaderSource, /relative order-2 min-w-0 overflow-hidden md:order-1 md:overflow-visible[\s\S]*?event-details-type/)
  assert.match(inlineHeaderSource, /relative order-1 min-w-0 md:order-2[\s\S]*?event-details-date/)
  assert.match(inlineHeaderSource, /h-\[52px\] min-w-0 overflow-hidden rounded-xl border border-\[#E5E7EB\][\s\S]*?md:contents/)
  assert.match(inlineHeaderSource, /appearance-none pr-10 md:appearance-auto md:pr-3/)
  assert.match(inlineHeaderSource, /max-w-full cursor-pointer border-transparent bg-transparent pl-12 pr-10 text-left text-transparent shadow-none \[min-inline-size:0\]/)
  assert.match(inlineHeaderSource, /text-left text-transparent[\s\S]*?md:text-stone-900/)
  assert.match(inlineHeaderSource, /left-12 right-10 top-1\/2[\s\S]*?formatDateInputForMobile\(eventForm\.date\)/)
  assert.match(inlineHeaderSource, /function formatDateInputForMobile\(value: string\)/)
  assert.match(inlineHeaderSource, /relative order-3 col-span-2 min-w-0 md:col-span-1[\s\S]*?event-details-venue/)
  assert.match(inlineHeaderSource, /col-span-2 min-w-0 md:col-span-1/)
  assert.match(inlineHeaderSource, />\s*Event Name\s*</)
  assert.match(inlineHeaderSource, /mobileDateIsEmpty \|\| !eventForm\.date \? 'Event Date'/)
  assert.match(inlineHeaderSource, />\s*Event Type\s*</)
  assert.match(inlineHeaderSource, /isUnnamedDefaultType\(value\)/)
  assert.match(inlineHeaderSource, /isDefaultMobileDate\(value\)/)
  assert.match(inlineHeaderSource, /return toDateInputValue\(value\.date\) === toDateInputValue\(new Date\(\)\.toISOString\(\)\)/)
  ...
  assert.match(savedRouteSource, /eventVendorDisplays\.length === 0 \? 'Vendors'/)
  assert.match(savedRouteSource, /eventVendorDisplays\.length === 1 \? 'Vendor' : 'Vendors'/)
