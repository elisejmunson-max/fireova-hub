...
const navGroups: NavGroup[] = [
  {
    ...
    children: [
      ...
      { href: '/content-bank', label: 'Media Bank', icon: MediaIcon },
      { href: '/vendors', label: 'Vendor Directory', icon: FolderIcon },
      { href: '/insights', label: 'Insights', icon: HistoryIcon },
      { href: '/settings', label: 'Business Profile', icon: SettingsIcon },
    ],
  },
]
...
export default function Sidebar({ user }: SidebarProps) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-stone-800">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 overflow-hidden rounded-lg bg-ember-600 flex items-center justify-center flex-shrink-0">
            <FlameIcon className="w-4 h-4 text-white" />
          </div>
          <div>
            <p className="text-white font-semibold text-sm leading-none">Fireova Content</p>
            <p className="max-w-[165px] truncate text-stone-500 text-xs mt-0.5" title={user.email}>
          ...
          return (
            <div key={group.label} className="mb-1">
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-stone-200 hover:text-white hover:bg-stone-800/40 transition-colors group disabled:cursor-default disabled:hover:bg-transparent disabled:hover:text-stone-200"
              >
              ...
              {!groupCollapsed && (
                <div className="space-y-0.5 mt-0.5">
                  {group.children.map((child, i) => {
                          ...
                          {!subCollapsed && (
                            <div className="space-y-0.5 mt-0.5">
                              {child.items.map((item) => (
                                <NavLink key={`${item.label}-${item.href}`} {...item} indent />
                              ))}
                            </div>
                          )}
                        </div>
                    ...
                    return <NavLink key={`${child.label}-${child.href}`} {...child} />
                  })}
                </div>
              )}
  ...
  return (
    <>
      ...
      {/* Mobile top bar */}
      <div
        className="lg:hidden fixed top-0 left-0 right-0 z-50 min-h-12 bg-stone-950 border-b border-stone-800 px-4 pb-2 flex items-center justify-between gap-3"
        style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.5rem)' }}
      >
        <div className="flex min-w-0 items-center gap-2">
          <div className="w-6 h-6 overflow-hidden rounded-md bg-ember-600 flex items-center justify-center">
            <FlameIcon className="w-3.5 h-3.5 text-white" />
          </div>
          <div className="min-w-0">
            <p className="whitespace-nowrap text-white font-semibold text-sm leading-none">Fireova Content</p>
            <p className="mt-1 max-w-[230px] truncate text-[10px] leading-none text-stone-400" title={user.email}>
              {user.email} · {user.id.slice(0, 8)}
            </p>
          </div>
        </div>
        <button
          ...
          className="relative z-[60] flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-stone-900 text-stone-200 ring-1 ring-stone-700 hover:text-white"
      ...
      {mobileOpen && (
        <>
          ...
          />
          <aside
            className="lg:hidden fixed top-0 left-0 bottom-0 z-40 w-64 bg-stone-950"
...
function UploadIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 16V4m0 0L7 9m5-5 5 5M4 16v3a2 2 0 002 2h12a2 2 0 002-2v-3" />
    </svg>
  )
}
...
function ApprovalIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-5M5 5h14v14H5V5z" />
    </svg>
  )
}
...
function HistoryIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l2.5 2.5M5 5v5h5M5.7 15A7 7 0 105 10" />
    </svg>
  )
}
